/**
 * @file main.cpp
 * @brief A simple virtual processor simulator.
 *
 ...
 *  - ADDv <register> <value>: Add an immediate value to the specified register (with overflow protection)
 *  - ADDr <register> <register>: Add a register's value to another register (with overflow protection)
 *  - SUBv <register> <value>: Subtract an immediate value from the specified register (with underflow protection)
 *  - SUBr <register> <register>: Subtract a register's value from another register (with underflow protection)
 ...
 *  - LOAD <memory_address> <register>: Load a 16-bit value from memory at the given address into the register
 *  - STORE <memory_address> <register>: Store the 16-bit value from the register into memory at the given address
...
int main(const int argc, const char** argv) {
  if (argc >= HardcodedValues::get_minimal_program_arguments_number()) {
    const string program_file_path =
        argv[HardcodedValues::get_program_file_path_index()];
